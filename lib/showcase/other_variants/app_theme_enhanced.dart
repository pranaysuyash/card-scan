...
class AppPalettes extends ThemeExtension<AppPalettes> {
  ...
  static AppPalettes fallback(ColorScheme scheme) {
    ColorScheme accent(Color seed) => ColorScheme.fromSeed(
          seedColor: seed,
...
class AppTheme {
  ...
  static ThemeData _buildTheme({
    ...
    final ColorScheme successScheme = dynamicSuccess ??
        ColorScheme.fromSeed(
          seedColor: _successSeed,
          brightness: brightness,
        );

    final ColorScheme warningScheme = dynamicWarning ??
        ColorScheme.fromSeed(
          seedColor: _warningSeed,
          brightness: brightness,
        );

    final ColorScheme infoScheme = dynamicInfo ??
        ColorScheme.fromSeed(
          seedColor: _infoSeed,
          brightness: brightness,
        );

    final ColorScheme neutralScheme = dynamicNeutral ??
        ColorScheme.fromSeed(
    ...
    final Color scaffold = Color.alphaBlend(
      baseScheme.primary
          .withOpacity(brightness == Brightness.light ? 0.04 : 0.12),
      baseScheme.surface,
    );

    final Color cardColor = Color.alphaBlend(
      baseScheme.secondary
          .withOpacity(brightness == Brightness.light ? 0.05 : 0.18),
      baseScheme.surface,
    );
    ...
    return ThemeData(
      ...
      outlinedButtonTheme: OutlinedButtonThemeData(
        style: OutlinedButton.styleFrom(
          textStyle:
              textTheme.labelLarge?.copyWith(fontWeight: FontWeight.w600),
          side: BorderSide(
              color: baseScheme.outline.withOpacity(0.6), width: 1.4),
          shape: ContinuousRectangleBorder(borderRadius: defaultRadius),
          padding: const EdgeInsets.symmetric(horizontal: 24, vertical: 16),
        ),
      ),
      inputDecorationTheme: InputDecorationTheme(
        ...
        enabledBorder: OutlineInputBorder(
          borderRadius: defaultRadius,
          borderSide:
              BorderSide(color: baseScheme.outlineVariant.withOpacity(0.25)),
        ),
